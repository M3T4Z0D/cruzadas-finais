import { GridTemplate, ClueSlot, gridTemplates } from "./templates";
import { WordInfo, wordsDatabase } from "./dictionary";
...
function findIntersections(slots: ClueSlot[]): Intersection[] {
...
export function generateBoardForElo(targetElo: number, preferredTemplateId?: string): GeneratedBoard {
  ...
  const slots = JSON.parse(JSON.stringify(template.slots)) as ClueSlot[];
  const intersections = findIntersections(slots);

  let eloTolerance = 150;
  let attempts = 0;
  let success = false;
  let assignments: string[] = new Array(slots.length).fill("");

  function solve(slotIndex: number, candidatesBySlot: WordInfo[][]): boolean {
  ...
  if (!success) {
    console.warn("Backtracking failed. Using fallback board pre-generation.");
    return getFallbackBoard(template);
  }

  const cluesList: GeneratedBoard["clues"] = [];
...
function getFallbackBoard(template: GridTemplate): GeneratedBoard {
  const fallbackSolutions: Record<string, Record<string, string>> = {
    template_7x7: {
      h1: "JANELA",
      h2: "BRASIL",
      h3: "FLAUTA",
  ...
  const cluesList = template.slots.map(slot => {
    ...
    const selectedClue = (dictWord.clues && dictWord.clues.length > 0)
      ? dictWord.clues[Math.floor(Math.random() * dictWord.clues.length)]
      : (dictWord as any).clue || "Palavra padrão do sistema";
